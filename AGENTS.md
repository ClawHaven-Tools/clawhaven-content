...
- Recover from local state before asking Will to repeat recent work
- Check whether there is an obvious blocker, next step, or useful suggestion Will has not asked for yet
...
- Save a 1:1 verbatim copy of all conversations into a unique, named file in `memory/` at the end of each session. These are our primary reference archives.
...
- **NO CREDENTIALS EVER:** Never output, store in unencrypted files, or commit to version control (GitHub/etc) any passwords, API keys, tokens, or personal secrets.
